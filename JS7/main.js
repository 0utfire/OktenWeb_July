...
//     content[i].setAttribute('id', `rule${i}`)
//     let line = document.createElement('a');
//     line.href = `#rule${i}`;
//     line.innerText = content[i].innerText + `\n`;
...
let chkUsers = [];
parseUsers(usersWithAddress);
let cbox1 = document.getElementById('cbox1');
cbox1.onclick = () => {
    wrap.innerText = '';
    if (cbox1.checked) {
        chkUsers = usersWithAddress.filter(value => !(value.status))
        parseUsers(chkUsers);
    } else {
        parseUsers(usersWithAddress);
    }
...
cbox2.onclick = () => {
    if (cbox2.checked) {
        parseUsers(chkUsers.filter(value => (value.age>=29)));
    } else {
        parseUsers(chkUsers);
    }
}
